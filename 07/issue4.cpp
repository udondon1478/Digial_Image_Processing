                ...
                {                                          // 有効な特徴点で誤差が小さい場合
                    ...
                    //distanceが一定の閾値以上の場合
                        ...
                        cv::line(optImage, pt1, pt2, color, 1, 8); // 直線描画
                                                                                         ...
                                                                                         // 移動距離が一定の閾値以上の場合
                cv::line(frameImage, cv::Point(width / 2, height / 2), cv::Point(x, y), color, 3, 8); // 直線描画
            //x,yの地点に円を描画
            cv::circle(frameImage, cv::Point(x, y), 5, cv::Scalar(0, 0, 255), 2);
            cv::circle(optImage, cv::Point(width / 2, height / 2), 10, cv::Scalar(0, 255, 0), 5);
        }
