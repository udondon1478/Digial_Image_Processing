...
void drawlines(cv::Mat &frameImage, std::vector<std::vector<cv::Point>> points);
...
// マウス座標を格納する配列
std::vector<std::vector<cv::Point>> linePoints; // 線の点のリスト (複数の線の情報を格納)
int pointCount = 0; // 入力された点の数

bool drawing = false; // 線を描画中かどうか
int currentLineIndex = -1; // 現在描画中の線のインデックス

//左クリックでframeImage内にcv::lineで繋がれた線を描画する関数
//右クリックで描画を終了し，frameImageを表示する
void drawlines(cv::Mat &frameImage, std::vector<std::vector<cv::Point>> points)
{
    // 各線を順番に描画
    for (size_t i = 0; i < points.size(); ++i) {
        // 2点以上クリックされている場合のみ線を描画
        if (points[i].size() >= 2) {
            for (size_t j = 0; j < points[i].size() - 1; ++j) {
                cv::line(frameImage, points[i][j], points[i][j + 1], cv::Scalar(0, 0, 255), 2);
            }
        }
    }
...
void display()
{
    ...
    for (size_t i = 0; i < linePoints.size(); ++i) {
        glBegin(GL_LINE_STRIP); // 線描画開始
        glColor3f(1.0f, 0.0f, 0.0f); // 赤色
        for (const auto& point : linePoints[i]) {
            glVertex3f(point.x, point.y, 0.0f); // 各点を結ぶ
        }
        glEnd(); // 線描画終了
    }
...
void mouse(int button, int state, int x, int y)
{
    if (state==GLUT_DOWN) {
        ...
        if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
            drawing = true;
            currentLineIndex++;
            linePoints.resize(currentLineIndex + 1); // 新しい線の配列を追加
            linePoints[currentLineIndex].push_back(cv::Point(x, y));
        }
        // 右クリックで描画終了
...
void mouseCallback(int event, int x, int y, int flags, void *userdata)
{
    ...
    switch (event) {
        ...
        case cv::EVENT_LBUTTONUP:
            std::cout << "左ボタンを離した";
            drawing = false; // drawingフラグをfalseにする
            break;
    ...
    if (event == cv::EVENT_LBUTTONDOWN) {
        ...
        currentLineIndex++;
        linePoints.resize(currentLineIndex + 1); // 新しい線の配列を追加
        linePoints[currentLineIndex].push_back(cv::Point(x, y)); // 始点を追加
        pointCount++;
    }

    // マウスが動いた時の処理
    if (event == cv::EVENT_MOUSEMOVE && drawing) {
        linePoints[currentLineIndex].push_back(cv::Point(x, y)); // マウス座標を軌跡に追加
    }
}