...
void drawlines(cv::Mat &frameImage, std::vector<cv::Point> points);
...
void detectSwipe(cv::Mat &prevGray, cv::Mat &gray)
{
    ...
    cv::goodFeaturesToTrack(prevGray, prevPoints, 100, 0.3, 7); // トラックする特徴点を検出
    if (prevPoints.empty())
        return;
    ...
    for (size_t i = 0; i < status.size(); i++)
    {
        if (status[i])
        {
            sumX += (points[i].x - prevPoints[i].x);
            count++;
        }
    }
...
void mouseCallback(int event, int x, int y, int flags, void *userdata)
{
    ...
    {
        drawing = true;
        linePoints.clear();                    // 軌跡をクリア
        linePoints.push_back(cv::Point(x, y)); // 始点を追加
    }

    // マウスが動いた時の処理
    if (event == cv::EVENT_MOUSEMOVE && drawing)
    {
        linePoints.push_back(cv::Point(x, y)); // マウス座標を軌跡に追加
    }
}