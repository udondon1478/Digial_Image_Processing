...
void initGL()
{
    ...
    col[0] = 0.725; col[1] = 0.494; col[2] = 0.30; col[3] = 1.0;
...
//-------------------------------
    ...
    // 立方体の配置
    for (int i = 0; i < 24; i++) { // 縦のループ
        for (int j = 0; j < 32; j++) { // 横のループ
            // 画素値を取得
            int x = j * (imageSize.width / 32); 
            int y = i * (imageSize.height / 24); 
            cv::Vec3b pixel = frameImage.at<cv::Vec3b>(y, x);

            // 画素値を色に変換
            color[0] = pixel[2] / 255.0; // 赤色成分
            color[1] = pixel[1] / 255.0; // 緑色成分
            color[2] = pixel[0] / 255.0; // 青色成分
            color[3] = 1.0; // アルファ値

            glPushMatrix();  //行列一時保存
            // 立方体の中心座標を計算 (frameImage のサイズに合わせて調整)
            glTranslated((j - 16) * (imageSize.width / 32.0), (i - 12) * (imageSize.height / 24.0), 0.0);

            // 立方体を描画
            glMaterialfv(GL_FRONT, GL_DIFFUSE, color);  //拡散反射係数
            glMaterialfv(GL_FRONT, GL_AMBIENT, color);  //環境光反射係数
            glBegin(GL_QUADS);
                // 前面
                glNormal3f(0.0, 0.0, 1.0);
                glVertex3f(-(imageSize.width / 64.0), (imageSize.height / 48.0), (imageSize.width / 64.0)); 
                glVertex3f((imageSize.width / 64.0), (imageSize.height / 48.0), (imageSize.width / 64.0)); 
                glVertex3f((imageSize.width / 64.0), -(imageSize.height / 48.0), (imageSize.width / 64.0)); 
                glVertex3f(-(imageSize.width / 64.0), -(imageSize.height / 48.0), (imageSize.width / 64.0)); 
                // 後ろ面
