...
int main(int argc, const char *argv[])
{
    ...
    for (int i = 0; i < contours.size(); i++)
    {
        ...
        circularity = 4 * 3.14159265358979323846 * area / (length * length);
        printf("Circularity[%d]=%f\n", i, circularity);
        // 円形度が80%未満の場合、膨張処理を行う
        if (0.8 < 4 * 3.14159265358979323846 * cv::contourArea(contours[i]) / (cv::arcLength(contours[i], true) * cv::arcLength(contours[i], true)))
        {
            cv::erode(binImage, binImage, element, cv::Point(-1, -1), 100);
            cv::imshow("Gray", binImage);
    ...
    */
    ...
    /*
        ...
        for (int i = 0; i < contours.size(); i++)
        {
            ...
            if (0.8 < 4 * 3.14159265358979323846 * cv::contourArea(contours[i]) / (cv::arcLength(contours[i], true) * cv::arcLength(contours[i], true)))
            {
                cv::erode(binImage, binImage, element3);
                cv::erode(binImage, binImage, element4);

                cv::imshow("Gray", binImage);
                cv::waitKey(0);

                cv::dilate(binImage, binImage, element4);
                cv::dilate(binImage, binImage, element3);
                cv::imshow("Gray", binImage);
